...
namespace Il2CppDumper
{
    static class ArmUtils
    {
        ...
        public static ulong DecodeAdr(ulong pc, byte[] inst)
        {
            ...
            // net48 compat: string.Concat(ReadOnlySpan<char>) overload doesn't exist; use Substring.
            var uint64 = string.Concat(bin.Substring(8, 19), bin.Substring(1, 2));
        ...
        public static ulong DecodeAdrp(ulong pc, byte[] inst)
        {
            ...
            var bin = inst.HexToBin();
            // net48 compat: string.Concat(ReadOnlySpan<char>) overload doesn't exist; use Substring.
            var uint64 = string.Concat(bin.Substring(8, 19), bin.Substring(1, 2), new string('0', 12));
            uint64 = uint64.PadLeft(64, uint64[0]);
            return pc + Convert.ToUInt64(uint64, 2);
        }
